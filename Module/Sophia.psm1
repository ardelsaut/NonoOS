...
{
    Centrer "On installe le Paquet dont depend le script (NuGet et PSGallery)" Cyan
    ...
    Install-Module -Name "PowerShellGet" -Force -SkipPublisherCheck -ErrorAction Ignore
    Import-Module "PowerShellGet"
    Centrer "Le module (PowerShellGet) est installe!!!!" Green
...
{
    Centrer "On installe ChocoLatey" Cyan
    Set-ExecutionPolicy Bypass -Scope Process -Force; [System.Net.ServicePointManager]::SecurityProtocol = [System.Net.ServicePointManager]::SecurityProtocol -bor 3072; iex ((New-Object System.Net.WebClient).DownloadString('https://community.chocolatey.org/install.ps1'))
    choco feature enable -n=allowGlobalConfirmation
    choco feature enable -n=allowEmptyChecksums
	Centrer "On installe les Applications ChocoLatey"
# Deskpins
    choco install deskpins --ignore-checksums
# Files
    choco install files --ignore-checksums
# MusicBee
    choco install musicbee --ignore-checksums
# Obs Studio
    choco install obs-studio --ignore-checksums
# MPV.Net
    choco install mpvnet.install --ignore-checksums
# Battle.net
    choco install battle.net --ignore-checksums
...
function InstallGit
{
    ...
    Centrer "Git est installe!!!!" Green
    # RefreshEnvironment
    Centrer "On installe le module dont depend le script (Posh-Git)" Cyan
...
function AutoLogonWin
{
    Centrer "On active la Connexion a Windows automatique (autologon)" Cyan
    ...
    Centrer "Connexion a Windows automatique (autologon) est active!!!" Green
}
...
{
    Centrer "On clean un peu le dossier AppData de User avant de Commencer" Cyan
    ...
    Centrer "Dossier AppDate nettoyé!!!" Green
	
    Centrer "On Copie les dossiers de configurations disponnibles sur le Nas" Cyan
...
function DossierConfigAdobe
{
    ...
    Start-Sleep -Seconds 1
    H:\"Adobe Acrobat DC"\Setup.exe /sAll /rs /msi EULA_ACCEPT=YES
    Start-Sleep -Seconds 1
...
function DossierConfigVSCodium
{
    $ConfigDesktop = "$env:USERPROFILE\Desktop\Config"
	$configpath = "$env:USERPROFILE\.vscode-oss" 
    $pathExists = Test-Path -Path $configpath
    If ($pathExists)  {
    rmdir /s $env:USERPROFILE\.vscode-oss
    Copy-Item "$ConfigDesktop\2-vscodium\.vscode-oss\" -Destination "$env:USERPROFILE\" -Force -Verbose -Recurse
    }
    else {
    (Copy-Item "$ConfigDesktop\2-vscodium\.vscode-oss\" -Destination "$env:USERPROFILE\" -Force -Verbose -Recurse )
    } 
...
function DossierConfigOffice
{
    $ConfigDesktop = "$env:USERPROFILE\Desktop\Config"
    Copy-Item "$ConfigDesktop\03-Office\" -Destination "$env:USERPROFILE\A-INSTALLER\Office" -Force -Verbose -Recurse
    powershell.exe "$env:USERPROFILE\A-INSTALLER\Office\Office-x64_2k21_Ent\setup.exe" "/configure" "$env:USERPROFILE\A-INSTALLER\Office\Office-x64_2k21_Ent\configuration-Office2021Enterprise.xml"
    powershell.exe "$env:USERPROFILE\A-INSTALLER\Office\Online_KMS_Activation\Activate.cmd" "/o"
    powershell.exe "$env:USERPROFILE\A-INSTALLER\Office\Online_KMS_Activation\Activate.cmd" "/rat"
}
...
{
    $ConfigDesktop = "$env:USERPROFILE\Desktop\Config"
    Copy-Item "$ConfigDesktop\07-Scripts-Maisons\" -Destination "$env:USERPROFILE\Documents\Applications\Scripts-Maisons\" -Force -Verbose -Recurse
    Centrer "dossiers de configurations du Bureau vers la bonne Location de Chacun copie correctement!!!" Green
    Centrer "Dossier de Configuration COPIE depuis le Server vers L'ordinateur local!!!" Green
}
...
function DossierConfigStartisBack
{
    $ConfigDesktop = "$env:USERPROFILE\Desktop\Config"
...
function InstallByWinGet
{
	...
	Add-ProvisionedAppxPackages "$ConfigDesktop\14-Install-Winget-Manual\Microsoft.UI.Xaml.2.7.appx"
	Start-Sleep -Seconds 5
	Add-ProvisionedAppxPackages "$ConfigDesktop\14-Install-Winget-Manual\Microsoft.VCLibs.x64.14.00.Desktop.appx"
	Start-Sleep -Seconds 5
	Add-ProvisionedAppxPackages "$ConfigDesktop\14-Install-Winget-Manual\1.msixbundle"
    ...
    Start-Sleep -s 5
    Centrer "WinGet Installe!!!" Green

    Centrer "On installe Toutes les Applications disponnibles sur WinGet..."

    function winget-install($str1){
	...
	Remove-Item -Path "$env:USERPROFILE\Desktop\NonoOS\get-pip.py" -Recurse -Force -ErrorAction Ignore
	Remove-Item -Path "$env:USERPROFILE\Desktop\Flameshot.lnk" -Recurse -Force -ErrorAction Ignore
	Remove-Item -Path "$env:USERPROFILE\Desktop\GitHub Desktop.lnk" -Recurse -Force -ErrorAction Ignore
	Remove-Item -Path "$env:USERPROFILE\Desktop\Google Assistant.lnk" -Recurse -Force -ErrorAction Ignore
	Remove-Item -Path "$env:USERPROFILE\Desktop\Obsidian.lnk" -Recurse -Force -ErrorAction Ignore
...
# Pause Updates to 2050
   Set-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\WindowsUpdate\UX\Settings' -Name 'PauseUpdatesExpiryTime' -Value "2050-03-17T00:29:34Z"
}
...
function MenuRedemarrage
{
# Menu de Redémarrage
    $Title = "Fin de la configuration"
    $Prompt = "Installation terminee. Voulez-vous redemarrer?"
    $Choices = [System.Management.Automation.Host.ChoiceDescription[]] @("&Oui", "&Non")
    $Default = 0
    $Choice = $host.UI.PromptForChoice($Title, $Prompt, $Choices, $Default)
    switch($Choice)
    {
        0 { Restart-computer -Force -Confirm:$false}
        1 { Centrer "Installation terminee, il faudrait redemarrer"; pause}
    }
}
...
<#
	...
	Current user
#>
function WindowsCapabilitiesbynono
{
Remove-WindowsCapability -Name "Microsoft.Windows.WordPad~~~~0.0.1.0" -Online
Remove-WindowsCapability -Name "Microsoft.Windows.Notepad.System~~~~0.0.1.0" -Online
...
function UninstallUWPAppsbynono
{
# Get-AppxPackage "Microsoft.HEVCVideoExtension" -Verbose | remove-appxpackage -AllUsers -Verbose
# Get-AppxPackage "Microsoft.DesktopAppInstaller" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.StorePurchaseApp" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.WindowsNotepad" -Verbose | remove-appxpackage -AllUsers -Verbose
# Get-AppxPackage "Microsoft.WindowsStore" -Verbose | remove-appxpackage -AllUsers -Verbose
# Get-AppxPackage "Microsoft.WindowsTerminal" -Verbose | remove-appxpackage -AllUsers -Verbose
# Get-AppxPackage "Microsoft.WindowsTerminalPreview" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "AdvancedMicroDevicesInc-2.AMDRadeonSoftware" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "AppUp.IntelGraphicsControlPanel" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "AppUp.IntelGraphicsExperience" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.MicrosoftStickyNotes" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.ScreenSketch" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.Windows.Photos" -Verbose | remove-appxpackage -AllUsers -Verbose
# Get-AppxPackage "Microsoft.Photos.MediaEngineDLC" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.WindowsCamera" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.XboxIdentityProvider" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.XboxApp" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.GamingApp" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.GamingServices" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.Paint" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.Xbox.TCUI" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.XboxSpeechToTextOverlay" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.XboxGamingOverlay" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "Microsoft.XboxGameOverlay" -Verbose | remove-appxpackage -AllUsers -Verbose
Get-AppxPackage "NVIDIACorp.NVIDIAControlPanel" -Verbose | remove-appxpackage -AllUsers -Verbose
# Get-AppxPackage "Microsoft.WindowsCalculator" -Verbose | remove-appxpackage -AllUsers -Verbose
# Get-AppxPackage "RealtekSemiconductorCorp.RealtekAudioControl" -Verbose | remove-appxpackage -AllUsers -Verbose
# Get-AppxPackage "Microsoft.WebMediaExtensions" -Verbose | remove-appxpackage -AllUsers -Verbose
	}
